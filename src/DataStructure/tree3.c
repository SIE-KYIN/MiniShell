...
void	_pre_traverse(t_tree_node *root, t_list *env, char parent, t_info *info)
{
	int	backup_std[2];
	int	flag;
	...
	else if (is_redirect(root->command[0]))
	{
		...
		if (redir_out(root, root->right, flag) != -1)
			if (redir_in(root, root->right, flag, info) != -1)
				_pre_traverse(root->left, env, root->command[0][0], info);
...
void	pre_traverse(t_tree *tree, t_list *env, t_info *info)
{
	t_tree_node	*root;
